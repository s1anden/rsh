...
- **`executor.rs`** — Walks the validated AST, expands words (variables, globs, command substitution), wires pipes between pipeline stages, handles loops/conditionals, spawns processes with sanitized environment. **The executor is the security boundary for dynamic values** — it re-validates command names, checks expanded arguments for absolute paths and `..` traversal, re-checks blocked flags, and validates redirect targets, all post-expansion. It injects `-c /dev/null` into every ast-grep invocation (`ast_grep_args`), because ast-grep otherwise auto-discovers `sgconfig.yml` upward from its cwd and dlopen()s `customLanguages` libraries from it. Manages signal handling (SIGINT/SIGTERM) and output truncation.
